...
Detalle de implementación de `pharmacheck`. Para la guía de uso, ver [README.md](README.md).
...
python3 check_stock.py --solo FarmaPlus         # una sola farmacia (coincidencia parcial)
python3 check_stock.py --productos catalogo.csv --salida reportes/
```

| flag | default | qué hace |
|------|---------|----------|
| `--productos` | `productos.csv` | CSV con tus productos |
| `--farmacias` | `farmacias.json` | JSON con las farmacias a consultar |
| `--salida` | `reportes/` | carpeta donde se escriben los CSV |
| `--solo` | — | consultar sólo estas farmacias |
| `--umbral` | `0.65` | confianza mínima para aceptar un match por nombre |
| `--demora` | `0.4` | pausa en segundos entre pedidos a una misma farmacia |
| `--sep` | `;` | separador del CSV de salida |
| `--verbose` / `-v` | — | avance producto por producto |

Requiere Python 3.10+ y `requests`.

## Catálogo: `productos.csv`
...
- `stock-AAAAMMDD-HHMM.csv` — detalle, una fila por producto × farmacia. Columnas:
  `codigo, producto, ean, farmacia, estado, tipo_match, confianza, revisar, nombre_encontrado,
  precio, precio_lista, stock, sku, url, detalle`.
- `matriz-AAAAMMDD-HHMM.csv` — pivote: productos en filas, farmacias en columnas.
